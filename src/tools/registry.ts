...
const imageGen: ToolDef = {
  ...
  name: "生图（AI 绘画）",
  description:
    "根据文字描述生成图片并发送（需先在「生图配置」页配置提供商与 Key）。【同意才调用】用户要求看图片/生成图片/换一张时，你可以按角色设定拒绝；一旦同意就必须调用本工具真实生成——绝不能只描述画面、编造图片地址或假装已生成。【不要写地址】图片生成后系统会自动附带在回复末尾。prompt 的具体写法见系统提示里的「能力触发（生图）」段（当前生效的提供商对应哪套就按哪套写，不要自己判断）。negative 与 aspect 可不填：负面词系统已固定，比例默认跟随「生图配置」页的出图尺寸（auto=按画面内容自动选）。",
  parameters: {
    type: "object",
    properties: {
